...
(function(){
  ...
  var templates = {
    "audio": '\n<!-- 音频{audio-id}源代码开始 -->\n'
      + '{title-html}<div style="margin:0px 0px 30px 0px;text-align:center"><audio src="{src}" {attrs}controls="controls" style="width:800px;max-width:100%"></audio></div>{descr-html}\n'
      + '<!-- 音频{audio-id}源代码结束 -->\n'
      + '<p>&nbsp;</p>\n',

    "video": '\n<!-- 视频{video-id}源代码开始 -->\n'
      + '{title-html}<div style="margin:0px 0px 30px 0px;text-align:center;padding:0px 2%"><video src="{src}" poster="{poster}" {attrs}preload="none" controls="controls" webkit-playsinline="" playsinline="" style="background-color:black;width:800px;max-width:100%;"></video></div>{descr-html}\n'
      + '<!-- 视频{video-id}源代码结束 -->\n'
      + '<p>&nbsp;</p>\n',
  ...
  var renderTextComponent = function(info, key, templates) {
    var val = info[key];
  ...
  var renderTitle = function(info)
  {
    return renderTextComponent(info, "title", titleTemplates);
  };

  var renderDescr = function(info)
  {
    var s0 = info["descr"];
    if ( s0 === undefined ) {
      return "";
